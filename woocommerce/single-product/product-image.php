...
if ( has_post_thumbnail() ) {
	...
	if ( $attachment_count > 0 ) {
		...
		foreach($image_ids as $image_id) {
			$image_link = wp_get_attachment_url( $image_id );
			$output .= '<div class="slider-item"><img src="'.esc_url($image_link).'" alt="'.esc_attr__('thumb', 'beoreo').'"/></div>';
		}
		$output .= '</div>';
		echo ''.$output;
	} else {
	...
	echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<img src="%s" alt="%s" />', wc_placeholder_img_src(), __( 'Placeholder', 'beoreo' ) ), $post->ID );
}
