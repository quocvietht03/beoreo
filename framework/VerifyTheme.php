 ...
 */
define("ITEM_ID","15884077"); // Replace it by current item id
...
class VerifyTheme {
    ...
    // add style admin
      ...
      if(!$this->isInstallationLegit){
        $setting_page = admin_url('options-general.php?page=verifytheme_settings');
        $message = esc_html__( "Important notice: In order to receive all benefits of our theme, you need to activate your copy of the theme. \nBy activating the theme license you will unlock premium options - import demo data, install, update plugins and official support. Please visit Envato Settings page to activate your copy of the theme", 'beoreo' );
    ...
    public function verifytheme_settings_page(){
        ...
        $other_attributes = '';
        $register_button_text = __( 'Register your theme', 'beoreo' );
        if ( $toolkitData && $installationLegit ){
          $other_attributes = 'disabled';
          $register_button_text = __( 'Activated on this domain', 'beoreo' );
        ...
        <div class="wrap verifytheme_wrap">
            <form class="verifytheme_settings_form" method="post" action="options.php">
              ...
              <p class="change_license_wrap">
                <input name="change_license_tmp" onclick="document.getElementById('change_license_btn').click();" id="change_license_tmp" class="button" value="<?php esc_attr_e('Deregister your product','beoreo'); ?>" type="button">
              </p>
            <?php endif; ?>
            </form>
            <form style="display: none" id="change_license_form" method="POST">
              <button id="change_license_btn" type="submit" class="button button-primary" name="change_license"><?php echo esc_html__( 'Deregister your product', 'beoreo' ); ?></button>
            </form>
        </div>
        <?php
    }
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if(!empty($message)):
          ...
          if ( ! $already_in_use ):
            ...
            $message .= sprintf(wp_kses( __( 'This product is in use on another domain: <span>%s</span><br />', 'beoreo' ), array( 'span' => array(), 'br' => array() ) ), $connected_domain );
            $message .= sprintf(esc_html__('Are you using this theme for a new site? Please purchase a %s ', 'beoreo' ), '<a tabindex="-1" href="' . esc_url( 'http://themeforest.net/cart/add_items?ref=bearsthemes&item_ids=' ) .ITEM_ID.'" target="_blank">'.esc_html__('new license','beoreo').'</a>');
            if(!$register_error):
              add_settings_error(
                  'verifytheme_settings',
                  esc_attr( 'settings_updated' ),
    ...
    public function print_section_info()
    {
        printf(
            ...
            esc_html__('Themeforest provides purchase code for each theme you buy, and you’ll need it to verify and register your product (and to receive theme support).','beoreo'),esc_html__('To download your purchase code, simply follow these steps at ','beoreo'), esc_url('//bearsthemespremium.com/product-registration/'), esc_html__('here','beoreo')
        );
    }
    /**
    ...
    public function verifytheme_purchase_code_callback()
    {
        printf(
            ...
            isset( $this->options['purchase_code'] ) ? esc_attr( $this->options['purchase_code']) : '', esc_html__('Please insert your Envato purchase code. ','beoreo'), esc_url('//bearsthemespremium.com/product-registration/'), esc_html__('More info','beoreo')
        );
    }
}
